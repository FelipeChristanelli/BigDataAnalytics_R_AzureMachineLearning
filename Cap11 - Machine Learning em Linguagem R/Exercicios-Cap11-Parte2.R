...
# Obtendo apenas as colunas numéricas – EM AZUL SÃO SCRIPTS QUE NÃO COLOQUEI
...
summary(df) # Verificamos que não é distribuição normal
summary(df$G3)

hist(df$G3, main = "Histograma", xlab='Notas') # Mas é quase

# Matriz de Correlação - Variáveis Numéricas
correlação <- cor(df[c("age", "Medu", "Fedu", "traveltime", "studytime", "failures", "famrel", "freetime", "goout", "Dalc", "Walc", "health", "absences", "G1", "G2", "G3")])
View(correlação)

pairs(df[c("age", "Medu", "Fedu", "traveltime", "studytime", "failures", "famrel", "freetime", "goout", "Dalc", "Walc", "health", "absences", "G1", "G2", "G3")])

library(psych)
pairs.panels(df[c("age", "Medu", "Fedu", "traveltime", "studytime", "failures", "famrel", "freetime", "goout", "Dalc", "Walc", "health", "absences", "G1", "G2", "G3")])

# Dividino dados de Treino e Testes
str(ver)
...
# Previsão
previsão <- predict(modelo)
View(previsão)

# Prevendo gastos com dados de testes
prevtestes <- predict(modelo, teste)
View(prevtestes)
