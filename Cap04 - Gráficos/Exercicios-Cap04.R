...
mat
apply(mat, 1, mean) #1 significa que fará aplicação por linha
...
apply(escola[, c(2), drop=F], 2, mean) #primeira virgula vazia significa todas as linhas
# c(2) = coluna 2
...
mat1
barplot(mat1, beside= T) # para barras individuais, colocar o beside = T (TRUE)
...
ggplot(data = diamonds, aes (x=price, group = cut, fill = cut)) +
  geom_density(adjust = 1.5) #correto
